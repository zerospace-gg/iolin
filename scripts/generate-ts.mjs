...
async function generateEntityFile(jsonFilePath, entity) {
  ...
  const content = `import type { ${tsType} } from "../gg-iolin";
...
function generateIdxFile(idxData) {
...
function generateMetaFile(data, fileName, typeName) {
...
async function main() {
  try {
    ...
    const metaFiles = ["tags", "release", "idx", "all", "tagged"];
    ...
    const entityPromises = entityJsonFiles.map(async (jsonFile) => {
      try {
        ...
        if (!entity.type || !entity.slug) {
          console.error(`Invalid entity in ${jsonFile.name}: missing type or slug`);
          return;
        }

        // Generate TypeScript content
        const tsContent = await generateEntityFile(jsonFile.path, entity);
    ...
    try {
      ...
      Object.entries(idxData.all).forEach(([id, summary]) => {
        const tsType = getTypeScriptTypeFromSummary(summary.type, summary.subtype);
      ...
      const collectionPromises = Object.entries(typeGroups).map(async ([typeKey, groupData]) => {
        const fileName =
          typeKey === "add-on"
            ? "addons"
        ...
        const collectionContent = generateCollectionFile(typeKey, groupData, idxData.all);
      ...
      // Generate idx.ts
      const idxContent = generateIdxFile(idxData);
      const formattedIdxContent = await formatWithPrettier(idxContent);
      await fs.writeFile(path.join(tsDir, "idx.ts"), formattedIdxContent);
      console.log(`Generated: ${path.join(tsDir, "idx.ts")}`);
    ...
    const metaPromises = metaFileConfigs.map(async ({ file, type }) => {
      ...
      try {
        ...
        const content = generateMetaFile(data, file, type);
        const formattedContent = await formatWithPrettier(content);
        await fs.writeFile(path.join(tsDir, `${file}.ts`), formattedContent);
        console.log(`Generated meta file: ${path.join(tsDir, `${file}.ts`)}`);
      } catch (error) {
